...
const RecipientSidebar = ({
  ...
  if (loading)
    return <div className="p-4 animate-pulse">Loading categories...</div>;
  if (error)
    return (
      <div className="p-4 text-red-600">Error loading categories: {error}</div>
    );
          ...
          <ul className="space-y-3">
            ...
            {allRecipients.map((recipient, index) => (
              <li key={index}>
                <button
                  className={`text-sm w-full text-left px-3 py-2 rounded-lg transition-colors ${
                    selectedRecipient === recipient.recipients
                      ? "bg-red-100 text-red-600 font-medium"
                      : "hover:bg-red-50"
                  }`}
                  onClick={() => {
...
export const GiftShopPage = () => {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const recipientMatch = selectedRecipient
      ? product.recipients.includes(selectedRecipient)
      : true;
