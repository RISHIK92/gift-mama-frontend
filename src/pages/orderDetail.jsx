...
export const OrderDetail = () => {
    ...
    try {
      ...
      data.orderItems.forEach((item) => {
        if (
          ...
          Array.isArray(item.customizationImageUrl)
        ) {
  ...
  const CustomizationImageCarousel = ({ item }) => {
    if (
      !item.customizationImageUrl ||
      !Array.isArray(item.customizationImageUrl) ||
      item.customizationImageUrl.length === 0
    ) {
      return null;
    }
    ...
    return (
      <div className="mt-2 md:mt-0 md:ml-6 flex flex-col items-center">
        <p className="text-sm font-medium text-gray-700 mb-2">
          Customization {activeIndex + 1}/{imageCount}
        </p>
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="bg-white shadow-md rounded-lg p-6">
            ...
            {order.orderItems.map((item) => (
              <div
                key={item.id}
                className="flex flex-col md:flex-row md:items-center mb-4 pb-4 border-b border-gray-200 last:border-b-0"
              >
                <div className="flex flex-1 items-center mb-4 md:mb-0">
                  <img
                    src={
                      item.product.images[0]?.mainImage ||
          ...
          <div className="bg-white shadow-md rounded-lg p-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm">
                ...
                {renderStatusBadge(order.delivery)}
              </p>
              <p className="text-sm">
                <strong>Payment ID:</strong> {order.razorpayPaymentId || "N/A"}
              </p>
              <p className="text-sm">
